...
from LMS_FINAL.LMS import logout
...
def libr():
   ...
   # pic_0 = PhotoImage(file=r"C:\Users\gauta\Desktop\Codekul\library.png")
    ...
    name.place(relx=0.02, rely=0.15, relwidth=0.1, relheight=0.1)
    name1 = Label(root, text=aa[0], fg='black', bg='#fff', font=('Arial', 10, 'bold'))
    name1.place(relx=0.1, rely=0.15, relwidth=0.15, relheight=0.1)
    # ------------------------date-------------------------
    today = datetime.date.today()
    dat = Label(root, text='Date : ', bg='#fff', fg='black', font=('Arial', 10, 'bold'))
    dat.place(relx=0.7, rely=0.15, relwidth=0.1, relheight=0.1)
    dat2 = Label(root, text=today, bg='#fff', fg='black', font=('Arial', 10, 'bold'))
    dat2.place(relx=0.8, rely=0.15, relwidth=0.1, relheight=0.1)
    # cur()
...
#    s_pic_1 =pic_1.subsample(1,1)
 #   btn1.config(image=s_pic_1)
    ...
    btn7.place(relx=0.1, rely=0.25, relwidth=0.1, relheight=0.05)
    root.mainloop()
...
def home():
    try:
        ...
        window=Tk()
        window.title('Welcome To Gurukul')
        #root.title("My Library")
        window.iconbitmap("aa.ico")

        lbb = Label(window, bg='#0f624c')
        lbb.place(x=15, y=5)
        ig = PhotoImage(file='library.png')
        lbb.config(image=ig)
